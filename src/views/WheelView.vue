<template>
    <main class="container py-4">
            ...
            <div v-if="showPopup && currentItem" class="popup-overlay" @click.self="closePopup">
                <div class="popup-card">
                    ...
                    <div class="popup-badge">
                        ...
                        /
                        {{ selectedItems.length + remainingItems.length }}
                    </div>


                    <!-- COUNTDOWN -->
                    <div class="result-duration" :class="{ 'countdown-warning': countdown <= 5 }">
                        ⏱ {{ countdown }} seconds
                    </div>
                    ...
                    <div v-if="currentItem.kanji" class="popup-kanji mt-3">
                        <span class="popup-kana-label">
                            Kanji
                        </span>

                        <span class="popup-kanji-text">
                            {{ currentItem.kanji }}
                        </span>
                    </div>
                    ...
                    <div class="popup-kana">
                        ...
                        <span class="popup-kana-text japanese text-capitalize">
                            {{ currentItem.meaning }}
                        </span>
        ...
        <transition name="pop">
            <div v-if="showHistory" class="popup-overlay" @click.self="showHistory = false">
                <div class="popup-card" style="
                    ...
                    <button class="popup-close" @click="showHistory = false">
                        ×
                    </button>
                    ...
                    <p class="text-muted small mb-3 text-center">
                        {{ selectedItems.length }} items so far — wheel continues
                    </p>
                    ...
                    <div style="
                        ...
                        <div v-for="(item, i) in selectedItems" :key="i"
                            class="d-flex align-items-center gap-2 p-2 border-bottom" :class="{
                                'bg-light':
                                    currentItem &&
                                    currentItem === item
                            }">

                            <div class="fw-bold text-muted text-center" style="width: 28px;">
                                {{ i + 1 }}
                            </div>


                            <div class="japanese fw-bold" style="
                                    min-width: 70px;
                                    font-size: 1.2rem;
                                ">
                                {{ item.kanji }}
                            ...
                            <div v-if="currentItem && currentItem === item" class="badge bg-primary">
                                now
                            </div>

                        </div>

                        <div v-if="selectedItems.length === 0" class="p-4 text-center text-muted">
                            No picks yet
                        </div>
                    </div>
        ...
        <div v-if="
            ...
            !isSpinning
        " class="card mx-auto mt-4" style="max-width: 900px;">
...
const numQuestions = computed(
    () => Number(route.query.items) || 10
)

const durationQuestions = computed(
    () => Number(route.query.duration) || 20
)

const lessonStart = computed(
    () => Number(route.query.start) || 1
)

const lessonEnd = computed(
    () => Number(route.query.end) || lessonStart.value
)
...
const startCountdown = () => {
    clearCountdown()
    countdown.value = durationQuestions.value
...
const updateWheelSize = () => {
    ...
    if (vw < 380) {
        wheelSize.value = Math.floor(vw * 0.88)
    } else if (vw < 576) {
        wheelSize.value = Math.floor(vw * 0.85)
    } else if (vw < 768) {
        wheelSize.value = Math.floor(vw * 0.70)
    } else {
        wheelSize.value = 600
    }
}
...
const generateVocabulary = () => {
    const available =
        allVocabulary.value.filter(
            i =>
                i.lesson >= lessonStart.value &&
                i.lesson <= lessonEnd.value
        )
